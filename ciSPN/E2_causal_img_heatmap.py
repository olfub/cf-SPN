...
if __name__ == "__main__":
    ...
    gradcam_path = gradcam_dir / f"{conf.sample_id}_{conf.model_name}_{conf.loss_name}.png" # FIXME add loss2 and factor to path if needed

    experiment_name = get_experiment_name(conf.dataset, conf.model_name, conf.seed, conf.loss_name, conf.loss2_name, conf.loss2_factor, E=2)
    ...
    if cli_args.dataset == "hiddenObject":
        ...
        dataset = HiddenObjectDataset(hidden_object_base_dir, image_transform=image_transform, split=dataset_split)
    else:
    ...
    if conf.explicit_load_part == 'ciCNNSPN':
        spn, _, _ = load_spn(num_condition_vars, load_dir=load_dir, nn_provider=create_cnn_for_spn)
        eval_wrapper = spn

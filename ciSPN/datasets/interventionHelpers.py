...
intervention_vector_providers = {
    'CHC': lambda intervention: get_intervention_vector(intervention, intervention_vars_dict['CHC']),
    'ASIA': lambda intervention: get_intervention_vector(intervention, intervention_vars_dict['ASIA']),
    'CANCER': lambda intervention: get_intervention_vector(intervention, intervention_vars_dict['CANCER']),
    'EARTHQUAKE': lambda intervention: get_intervention_vector(intervention, intervention_vars_dict['EARTHQUAKE']),
    'TOY1': lambda intervention: get_intervention_vector(intervention, intervention_vars_dict['TOY1']),
    'TOY2': lambda intervention: get_intervention_vector(intervention, intervention_vars_dict['TOY2'])
}
...
class InterventionProvider:
    ...
    def __call__(self, path, data, known_intervention):
        ...
        intervention_vector = self.intervention_graph_provider(intervention_str).flatten()
        ...
        if known_intervention:
            ...
            intervention_info = np.tile(intervention_vector, (len(data[self.reference_var]), 1))
            interv_value = np.zeros_like(data[self.reference_var]) if intervention_var is '' else data[intervention_var]
            data[self.field_name] = np.concatenate((intervention_info, interv_value), axis=1)
        else:

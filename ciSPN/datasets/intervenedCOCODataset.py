...
class IntervenedCOCODataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, split="train", image_transform=None):
        ...
        if split == "train":
            self.coco = CocoDetection(root_dir+"train2017/", root_dir + 'annotations/instances_train2017.json')
        elif split == "val":
            self.coco = CocoDetection(root_dir+"val2017/", root_dir + 'annotations/instances_val2017.json')
        else:
    ...
    def __getitem__(self, item):
        ...
        if len(target) == 0:
            ...
            intervention_cat = coco_categories.index(coco_labels[target[intervention_idx]["category_id"]][-1])

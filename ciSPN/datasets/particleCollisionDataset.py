...
def file_name_to_intervention(file_name, data, time_steps, frames_per_sample):
    nr_variables = data.shape[1]//time_steps
    intervention_vector = np.zeros((frames_per_sample, nr_variables))
    intervention_str = file_name.split("_")[1]
    if intervention_str == "None":
        return intervention_vector
    intervention_str = intervention_str.split("=")[0]  # the intervention type (binomial, uniform,...) is not used here
    intervention_str = intervention_str[4:-1]  # remove "do" and brackets: two at the start and one at the end
    intervention_str, int_time = intervention_str.split(")")
    int_time = int(int_time)
    if "v" in intervention_str:
        # "vx" or "vy"
        int_attr = intervention_str[:2]
        int_particle = intervention_str[2:]
    else:
        int_attr = intervention_str[:1]
        int_particle = intervention_str[1:]
    int_attr = attr_to_index[int_attr]
    int_particle = int(int_particle)
    intervention_index = int_particle*4
    intervention_index += int_attr
    intervention_vector[int_time][intervention_index] = 1
    return intervention_vector
...
class ParticleCollisionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        target_vars = self._full_data[item, self.num_variables:self.num_variables*2:]
        ...
        return np.concatenate((observed_vars, intervention_vector, np.array([intervention_value]))), target_vars

...
parser.add_argument("--dataset", choices=["CHC", "ASIA", "CANCER", "EARTHQUAKE", "TOY1", "TOY2"], default="CHC") # CausalHealthClassification
...
experiment_name = get_experiment_name(conf.dataset, conf.model_name, conf.known_intervention, conf.seed, conf.loss_name,
...
dataset = TabularDataset(dataset_paths, X_vars, Y_vars, conf.known_intervention, conf.seed, part_transformers=providers)
...
loss, loss_ispn = create_loss(conf.loss_name, conf, num_condition_vars,
                              load_dir=runtime_base_dir / get_loss_path(conf.dataset, conf.known_intervention,
                                                                        conf.loss_load_seed))

if conf.loss2_name is not None:
    loss2, loss2_ispn, = create_loss(
        conf.loss2_name, conf, num_condition_vars,
        load_dir=runtime_base_dir / get_loss_path(conf.dataset, conf.known_intervention, conf.loss_load_seed))

...
class ClassifyStats:
    ...
    def eval(self, expected, prediction, print_samples=False):
        ...
        if print_samples:
            ...
            print("Expected:")
            print(expected[:8])
            elements, counts = torch.unique(predicted_classes, sorted=True, return_counts=True, dim=0)
            print("predicted stats (config,count):", elements, counts)
            elements, counts = torch.unique(expected_classes, sorted=True, return_counts=True, dim=0)

...
class RegressStats:
    ...
    def eval(self, expected, prediction):
        ...
        self._rel_error += torch.sum(torch.abs(expected_classes-predicted_classes) / torch.abs(expected_classes))

        rel_error_pf = torch.sum(torch.abs(expected_classes-predicted_classes) / torch.abs(expected_classes), dim=0)
        if self._rel_error_per_feature is None:
            self._rel_error_per_feature = rel_error_pf
        else:
            self._rel_error_per_feature += rel_error_pf

        correct = torch.all(torch.isclose(expected_classes, predicted_classes, atol=self._treshold), dim=1)
    ...
    def get_eval_result_str_per_feature(self):
        str1 = " ".join([f"{error/self._num_evals:.2f}" for error in self.get_error_per_feature()])
        str2 = " ".join([f"{error/self._num_evals:.2f}" for error in self.get_rel_error_per_feature()])

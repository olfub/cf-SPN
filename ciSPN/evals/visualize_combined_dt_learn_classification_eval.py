...
def tsplot(ax, data, **kw):
    ...
    ax.plot(x, est, **kw)
    plt.errorbar(x, est, sd, markersize=8, capsize=5) #, linestyle='None') #, marker='|')
    ax.margins(x=0)
...
for dataset, dataset_abrev in zip(datasets, dataset_abrevs):
    ...
    for factor in factors:
        file_names = list(glob.glob(f"../../{eval_folder}/D1_{dataset}_{loss}_{factor}_Seed*_eval_log.txt"))
        file_names.sort()
        #print(file_names)
    ...
    models["DT"] = {"mean": model_accs_mean, "std": model_accs_std, "accs": np.array(model_accs).T}

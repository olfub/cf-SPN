...
def get_experiment_name(dataset, model_name, known_intervention, seed, loss_name=None, loss2_name=None,
                        loss2_factor_str=None, E=1):
...
def get_loss_path(dataset_name, known_intervention, loss_load_seed):
    loss_folder = get_experiment_name(dataset_name, "ciSPN", known_intervention, loss_load_seed, "NLLLoss", None, None)
    return loss_folder


def create_loss(loss_name, conf, num_condition_vars=None, load_dir=None, nn_provider=None):
    loss_ispn = None
    if loss_name == "NLLLoss":
        ...
        loss_ispn, _, _ = load_spn(num_condition_vars, load_dir=load_dir, nn_provider=nn_provider)
        loss_ispn.eval()

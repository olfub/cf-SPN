...
def get_E2_experiment_name(dataset, model_name, seed, loss_name=None, loss2_name=None, loss2_factor_str=None):
...
def get_E2_loss_path(dataset_name, loss_load_seed):
    loss_folder = get_E2_experiment_name(dataset_name, "ciCNNSPN", loss_load_seed, "NLLLoss", None, None)
    return loss_folder
...
def create_cnn_model(num_condition_vars, num_target_vars):
    cnn = SimpleCNNModelC(
        num_sum_weights=None, num_leaf_weights=num_target_vars)
    return cnn


def create_cnn_for_spn(spn, num_condition_vars, num_sum_params, num_leaf_params):
    cnn = SimpleCNNModelC(num_sum_weights=num_sum_params, num_leaf_weights=num_leaf_params)
    return cnn
